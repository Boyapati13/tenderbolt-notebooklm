...
export default function SupportingDocumentsPage() {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const supportingDocs = data.documents?.filter((doc: any) => doc.category === 'supporting') || [];
      setDocuments(supportingDocs);
    } catch (error) {
  ...
  const uploadFiles = async (selected: File[]) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification(`✅ Successfully uploaded ${selected.length} supporting document(s)!`, "success");
    ...
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Supporting Documents</h1>
            <p className="text-gray-600 mt-1">
              Global library of certificates, licenses, and compliance documents
            </p>
          </div>
        </div>
        <button
          className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-200 hover:scale-105 shadow-lg"
        ...
        {filteredDocuments.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {documents.length === 0 ? "No supporting documents yet" : "No documents match your search"}
            </h3>
            <p className="text-gray-600 mb-4">
              Upload certificates, licenses, registrations, and compliance documents
            </p>
            <button
              className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-200 hover:scale-105 mx-auto"
              onClick={() => setShowUploadModal(true)}
            >
              <Plus size={16} />
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-green-600 font-medium">
                        {doc.documentType || 'Supporting Document'}
                      </p>
                      {doc.summary && (
